import { Context, APIGatewayProxyCallback, EventBridgeEvent } from 'aws-lambda'
...
async function publishEvent(event: OrderDeliveredEvent) {
  try {
    ...
    console.log(
      `Sending event ${JSON.stringify(event)} to ${orderDeliveredUri}`,
    )
...
export const lambdaHandler = (
  ...
  if (event?.detail == null) throw new Error('Event detail cannot be null')

  const now = new Date()
  // publish orderDelivered event
  const orderDelivered = publishEvent({
